...
\hline
Preconditions & TCP服务端已启动、MediaPipe模块已启动并开启手指指向捕捉功能、Unity交互应用已启动并处于可自由移动状态 \\
\hline
Description & 参展者伸出食指指向目标方向；MediaPipe 检测手部 21 个特征点坐标，计算食指 MCP 关节（点 5）至指尖 TIP（点 8）的连线方向向量；系统将该方向向量在水平面上归一化，映射为 Unity 的移动输入坐标并经 TCP 发送接入Unity Input System；Unity 实时更新旅人角色的位移向量，驱动其持续行走；摄像机保持平滑跟随。 \\
\hline
Postconditions & 旅人角色位置持续更新至参展者所指方向 \\
\hline
Notes & 移动速度恒定，方向随手势连续变化实时更新；仅当食指伸直且其余四指弯曲时识别为指向手势；手势识别到角色响应的端到端延迟控制在100ms以内 \\
\hline
\end{tabularx}
...
手势提供了空间指向性，而面部表情则承载了参展者的情绪信息。系统通过摄像头实时采集参展者面部图像，经MediaPipe FaceLandmarker提取468个面部关键点，计算七种基本情绪的分类标签与置信度，并在情绪状态发生变化时上报至EchoAgent，作为后续决策的情感上下文。此过程不涉及直接的交互触发，属于后台持续运行的数据采集链路，其用例描述如表\ref{tbl:usecase-expression}所示。
...
\hline
Preconditions & 摄像头正常工作，MediaPipe FaceLandmarker已初始化 \\
\hline
Description & MediaPipe FaceLandmarker持续采集摄像头画面，提取面部478个关键点坐标与52维面部形变系数（blendshapes）；系统实时计算形变系数均值作为情绪强度指标，当指标变化幅度超过预设阈值且距上次上报超过最小间隔时，经TCP将面部形变数据发送至EchoAgent；Agent基于形变系数推理参展者情绪状态，更新内部情感上下文 \\
\hline
Postconditions & Agent获得参展者当前面部形变数据（形变系数及其置信度、鼻尖坐标、时间戳） \\
\hline
Notes & 形变系数覆盖52种面部运动单元，为Agent的情绪推理提供细粒度底层特征；面部检测目标帧率≥15fps；均值变化阈值与上报最小间隔均可通过参数调整 \\
\hline
\end{tabularx}
...
前述两项用例关注外部感知输入，而交互应用内部的物体交互构成了叙事推进的核心机制。参展者通过手势引导旅人接近场景中的可交互物体（音符、机关触发器、叙事标记点等），Unity基于物理碰撞系统判定交互触发条件，进而执行对应的交互逻辑——收集音符以解锁章节进度、激活机关以改变场景结构，或触发叙事事件以推进情感故事线。关键交互事件同步上报至EchoAgent，使Agent感知参展者的叙事进度。其用例描述如表\ref{tbl:usecase-interaction}所示。
...
\hline
Description & 参展者通过手势引导旅人接近场景中的可交互物体（音符、触发器、叙事标记点）；Unity物理碰撞系统检测角色与物体的接触或距离进入判定阈值；Unity触发对应的交互逻辑（收集音符、激活机关、触发叙事事件）；Unity更新场景状态与交互进度；关键交互事件经TCP上报至EchoAgent \\
\hline
Postconditions & 场景交互状态更新，叙事进度推进至下一里程碑 \\
\hline
Notes & 交互判定基于Unity物理碰撞系统；不同物体挂载不同的交互脚本，通过统一的事件接口上报Agent \\
\hline
\end{tabularx}
...
上述感知输入——手势指向、表情变化与场景交互事件——最终汇聚至EchoAgent，由Agent完成跨模态的语义整合与意图推理。Agent以Qwen3.6 VLM为核心推理引擎，将多源感知数据与当前叙事状态统一为上下文表示，综合判断参展者的行为意图与情感趋向，生成结构化的决策输出。这一用例属于系统内部的处理链路，不涉及参展者的直接交互，却是连接感知与反馈的关键认知桥梁。其用例描述如表\ref{tbl:usecase-understanding}所示。
...
\hline
Preconditions & 感知层各模块正常运行，Agent与各模块TCP连接正常，叙事状态机已初始化 \\
\hline
Description & Agent接收来自MediaPipe的手势识别结果、表情识别结果及Unity的场景状态信息；Agent将多源数据整合为统一的上下文表示，关联当前叙事状态节点；Agent基于上下文进行意图推理与情感判断，评估是否触发状态转移；Agent根据推理结果确定交互策略（视觉参数调整、灯光模式切换、叙事状态推进）；Agent生成结构化的决策输出，供后续控制命令下发使用 \\
\hline
Postconditions & Agent完成场景状态理解，生成决策输出（目标视觉参数、灯光模式、叙事状态更新） \\
\hline
Notes & 决策基于Qwen3.6 VLM的视觉-语言联合推理能力，通过DeepAgent框架管理工具调用链；上下文窗口保留近30s内的关键交互历史 \\
\hline
\end{tabularx}
...
Agent完成场景理解与决策后，需将决策结果转化为执行端可解析的控制指令。系统采用自定义JSON协议，Agent将目标视觉参数、灯光模式与叙事状态更新封装为结构化指令，经TCP分别下发至Unity与树莓派，各执行端解析后执行并返回ACK确认。其用例描述如表\ref{tbl:usecase-command}所示。
...
\hline
Preconditions & Agent与各执行端TCP连接正常，决策输出已生成 \\
\hline
Description & Agent根据决策输出构造JSON格式的控制指令（目标着色器参数、灯光颜色与模式、叙事状态更新等）；指令经TCP分别发送至Unity与树莓派；Unity解析指令，更新场景渲染参数与叙事状态；树莓派解析指令，更新LED灯阵的灯光模式与颜色；各执行端完成更新后返回ACK确认 \\
\hline
Postconditions & Unity场景渲染参数与树莓派灯光状态按指令更新，Agent收到ACK确认 \\
\hline
Notes & 指令采用自定义JSON协议；若执行端超时未返回ACK，Agent将重试一次；重试失败则记入错误日志 \\
\hline
\end{tabularx}
...
Unity接收Agent下发的视觉参数后，在GPU端实时计算着色器效果，驱动虚拟场景的动态视觉呈现。场景中的关键视觉元素——山峦的抖动与固化、建筑的舒展与扭曲、潮水的涨落与光点浮动——均通过着色器参数插值实现平滑过渡，避免突变造成的视觉割裂。后处理栈同步调整色调映射与Bloom强度，使画面整体氛围与当前情感基调保持一致。其用例描述如表\ref{tbl:usecase-visual}所示。
...
\hline
Preconditions & Unity交互应用正常运行，场景资源已加载，着色器已预热编译 \\
\hline
Description & Unity解析Agent下发的视觉参数（着色器属性、后处理参数、粒子系统参数）；GPU端实时计算顶点/片元着色器，驱动山峦形变、建筑扭曲、水面波动等视觉效果；后处理栈根据情绪倾向调整色调映射、Bloom强度与色彩分级；粒子系统根据场景氛围调整发射速率、生命周期与运动轨迹；所有视觉变化通过Lerp/Slerp插值在0.5--2.0s内平滑过渡 \\
\hline
Postconditions & 虚拟场景视觉呈现更新至与当前情绪状态和叙事进度匹配 \\
\hline
Notes & 着色器参数插值采用Lerp/Slerp保证平滑过渡；视觉变化目标帧率≥30fps；后处理使用Unity Post-Processing Stack v2 \\
\hline
\end{tabularx}
...
虚拟场景的视觉变化通过实体LED灯阵延伸至物理展览空间，使沉浸感从屏幕内部扩散至真实环境。Agent根据场景理解结果确定目标灯光模式、颜色与亮度，通过树莓派驱动WS2812 LED灯阵执行对应效果，使实体空间的光影氛围与虚拟场景的视觉基调协调一致。其用例描述如表\ref{tbl:usecase-light}所示。
...
\hline
Preconditions & 树莓派已建立TCP连接，LED灯阵电源接通，灯光控制脚本已加载 \\
\hline
Description & Agent根据场景理解结果确定目标灯光模式（chase/solid/rainbow/breathing）、颜色与亮度参数；Agent将灯光控制指令经TCP下发至树莓派；树莓派通过rpi-ws281x库将指令转换为PWM信号；WS2812 LED灯阵执行对应灯光模式，实体空间氛围同步变化 \\
\hline
Postconditions & LED灯阵切换至指定灯光模式，实体空间与虚拟场景视觉氛围协调一致 \\
\hline
Notes & 支持chase/solid/rainbow/breathing四种预设灯光模式；灯光变化端到端延迟控制在200ms以内；LED灯阵共计256颗WS2812可寻址LED \\
\hline
\end{tabularx}
...
\hline
Preconditions & 系统运行中，日志模块正常工作，调试客户端与Agent建立TCP连接 \\
\hline
Description & 开发者通过调试客户端发送日志查询指令（指定时间范围、模块、日志级别）；Agent解析查询条件，检索对应日志记录；Agent将日志结果格式化后返回至调试客户端；开发者基于日志进行分析、调试或状态监控 \\
\hline
Postconditions & 开发者获取指定范围的系统运行日志 \\
\hline
Notes & 日志按模块和时间戳索引；支持按DEBUG/INFO/WARN/ERROR级别过滤；支持关键词模糊搜索 \\
\hline
\end{tabularx}
...
作为为《回声之境》服务的智能体，本文将其命名为 EchoAgent。EchoAgent 内部基于大语言模型构建，作为整个系统的理解层和中枢节点。TCP Socket 服务器与 EchoAgent 之间通过一个异步消息队列进行通信：服务器负责接收来自各类客户端（Unity、MediaPipe、Raspberry Pi 等）的消息，并将它们投入共享的消息队列；EchoAgent 则定时从队列中批量取出消息，经大语言模型理解后，自主决策并调用相应工具完成响应。
...
下面是一个完整的智能体参与的交互闭环过程实例。如图~\ref{fig:agent-trace} 的时序图所示，该流程从参展者触发交互事件开始——例如 MediaPipe 模块检测到参展者做出了挥手动作；挥手事件随即通过 TCP Socket 发送至智能体；智能体结合当前正在体验的章节、参展者进度、情绪等多源信息进行综合决策；若智能体认为需要查看应用现状以判断下一步动作，便会调用截图工具；截图请求经 TCP 发送至 Unity 客户端；Unity 执行截图操作后，将截图通过 TCP 回传至智能体；智能体理解截图内容后，发起下一步交互应用内事件，从而形成完整的"感知—理解—反馈"闭环。
...
\textbf{（1）系统架构描述模型}
 ...
 SKILL体系首先定义了系统的层次化架构模型。本系统采用“感知层—理解层—反馈层”的三层星型拓扑结构，其中感知层包含手势识别、面部情绪识别与交互应用状态同步三个独立模块，各模块均以TCP Client方式向理解层上报数据；理解层以Qwen3.6 VLM为基座模型，结合DeepAgent框架实现Tool Calling能力；反馈层则由Unity客户端与树莓派物理装置组成。SKILL中通过结构化字段明确各层的职责边界与数据流向，使智能体能够理解其在系统中的定位与作用。
