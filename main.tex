...
在确定概念之后，我使用Google推出的NanoBanana生成了旅人的形象以及其不同视图；然后根据不同视图图片，使用字节跳动推出的Seed 3D生成带PBR材质的OBJ格式3D模型；由于此旅人形象非标准人形，所以无法套用通用的人体骨骼模板，因此接下来，我将3D模型导入Blender，手工完成骨骼绑定和关键帧动画；最后要让旅人能够在Unity中被控制，还需要实现一套输入控制脚本和简单的状态机。
...
旅人角色的移动由 TravelerController 组件驱动，该组件依赖 Unity 的 CharacterController 进行物理位移计算。在 \texttt{FixedUpdate} 中按固定时间步长执行重力施加、输入解析、旋转平滑与位移更新：读取 Input System 的 Move 动作获取二维输入向量，结合主摄像机的偏航角计算世界空间移动方向，通过 \texttt{Mathf.SmoothDampAngle} 实现角色朝向的平滑过渡；垂直方向累计重力加速度，着地时清零。TravelerController 的核心移动逻辑如代码\ref{code:traveler-controller}所示。
