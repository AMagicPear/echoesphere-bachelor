...
首选介绍手势交互控制交互应用中的旅人移动的用例。在人们日常生活中，手势往往作为语言以外的一种高效的信息传递方式，点名、问路的时候人们都会下意识地用手指向自己所指代的物体，以至于“所指”这个词本身都已经用上了“指”这个字。如表\ref{tbl:usecase-gesture}所示。

\begin{table}[h]
\centering
\caption{手势交互控制角色用例结构化描述表}
\begin{tabularx}{\linewidth}{|c|X|}
\hline
Actors & 参展者、MediaPipe模块、Unity \\
\hline
Description & 参展者做出捏合手势后，MediaPipe检测指尖关键点距离并判定为捏合事件，经TCP上报至决策中枢；Agent综合当前章节与情绪状态决策触发何种音符，向Unity下发演奏指令，同步通知树莓派执行灯光伴随效果 \\
\hline
Date & 手部21点关键点坐标、捏合状态、当前章节与场景信息、情绪分类结果 \\
\hline
Stimulus & 参展者拇指尖与食指尖归一化距离低于0.06，触发捏合事件 \\
\hline
Response & 虚拟场景中触发音符演奏的视觉与音效反馈；实体LED灯阵同步闪烁对应灯光模式 \\
\hline
Comments & 不同场景下捏合手势映射至不同音符（水滴/道路交错/海浪/风）；手势识别到视觉反馈的端到端延迟控制在100ms以内 \\
\hline
\end{tabularx}
\label{tbl:usecase-gesture}
\end{table}

\textbf{用例二：情绪感知与场景适配。}面部情绪识别模块持续分析参展者面部关键点，将情绪状态变化通过TCP上报至EchoAgent；Agent据此调整场景视觉参数与灯光氛围。其结构化描述如表\ref{tbl:usecase-emotion}所示。

\begin{table}[h]
\centering
\caption{情绪感知与场景适配用例结构化描述}
\begin{tabularx}{\linewidth}{|l|X|}
\hline
\textbf{Actors} & 参展者、MediaPipe面部识别模块、EchoAgent、Unity交互应用、树莓派灯光控制器 \\
\hline
\textbf{Description} & 面部识别模块基于MediaPipe FaceLandmarker持续检测468个面部关键点，当情绪分类结果发生变化时，通过TCP上报至Agent；Agent根据情绪倾向（正向/负向）及强度，向Unity下发着色器参数调整指令，向树莓派下发灯光颜色与亮度调整指令 \\
\hline
\textbf{Date} & 面部468点关键点坐标、情绪分类标签与置信度、当前章节信息 \\
\hline
\textbf{Stimulus} & 面部识别模块检测到参展者情绪状态发生变化 \\
\hline
\textbf{Response} & Unity实时调整场景着色器的菲涅尔强度、抖动幅度等视觉参数；LED灯阵切换颜色基调与亮度 \\
\hline
\textbf{Comments} & 正向情绪触发暖色调（琥珀色/金色）+高亮度，负向情绪触发冷色调（蓝紫色）+低亮度；情绪变化至视觉响应的端到端延迟控制在200ms以内 \\
\hline
\end{tabularx}
\label{tbl:usecase-emotion}
\end{table}

\textbf{用例三：虚实联动灯光反馈。}Unity交互应用中的关键事件触发后，通过TCP上报至Agent，经决策后向树莓派下发灯光控制指令，驱动实体LED灯阵产生同步响应。其结构化描述如表\ref{tbl:usecase-light}所示。

\begin{table}[h]
\centering
\caption{虚实联动灯光反馈用例结构化描述}
\begin{tabularx}{\linewidth}{|l|X|}
\hline
\textbf{Actors} & Unity交互应用、EchoAgent、树莓派灯光控制器、WS2812 LED灯阵 \\
\hline
\textbf{Description} & Unity端触发关键交互事件（音符获取、章节切换、谜题完成等）后，通过TCP上报事件信息至Agent；Agent根据事件类型与当前状态决策灯光模式，向树莓派下发控制指令；树莓派通过PWM信号驱动WS2812灯带执行 \\
\hline
\textbf{Date} & 交互事件类型与参数、当前章节与场景、灯光模式预设（chase/solid/rainbow/breathing） \\
\hline
\textbf{Stimulus} & Unity交互应用中触发预设的关键交互事件 \\
\hline
\textbf{Response} & LED灯阵切换至对应灯光模式，与虚拟场景的视觉氛围保持同步 \\
\hline
\textbf{Comments} & 灯光指令经Agent决策后通过TCP下发，树莓派端使用rpi-ws281x库将PWM信号转换为LED显示；灯光变化延迟控制在200ms以内 \\
\hline
\end{tabularx}
