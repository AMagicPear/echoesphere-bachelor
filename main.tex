...
以下以第一章"初域·未定之原"为例，展示系统的完整交互流程与多模态反馈效果。

\subsection*{序幕}

交互应用启动后，初始界面显示Logo与"回声"标题。参展者点击开始后，画面缓缓放大，摄像机穿越层层迷雾——迷雾中浮现波形图案，暗示声音与世界之间的隐秘关联。摄像机持续前推，最终从云雾中显现出旅人在绵延山峦与旷野前行的画面。旁白响起：

\noindent\textit{"他是一个没有目的地的旅人。就好像时间总是推着钟表的指针前进，他也像是被什么推着，走着，走着。当他看到眼前绵延的山峦和旷野，他忽然想到——在不久的记忆里，那个带着他爬上山顶看到广阔世界，却在悬崖边将他抛下的晴天娃娃。"}

摄像机逐渐减速，停在旅人斜前方，平视旅人，留出背后辽阔的风景。风景上叠加第一章名称——\textbf{初域·未定之原}。

\subsection*{场景氛围}

这是世界最开阔、也最无形的地方。地貌不断变化，柔软的草浪起伏，半透明的山丘若隐若现，石块漂浮又坠落。这里没有威胁，只有迷茫的方向。其核心感悟为：有些地方之所以难走，是因为自己的"方向感"仍未成形；世界不会惩罚旅人，它只是保持未定，等待旅人决定它的形状。

\subsection*{交互流程}

\textbf{进入世界}。参展者开始接受移动输入，通过手势指向控制旅人离开出生点。当旅人离开出生点圆圈时，旁白响起："没有人知道旅人从何而来。"随后离开出生点石台，旁白继续："当他睁开眼时，正立于'未定之原'的浅雾中，风声像未完成的句子，光像正在犹豫的记忆。"此时画面中的风忽然吹得大了些又渐渐缓和，伴随呼呼的风声音效。

\textbf{获得水滴音符}。屏幕出现文字引导——"尝试让世界听见你的'形状'"——提示参展者按下水滴音符。交互应用内播放音符，画面中半透明山峦随之抖动。参展者用手势从画面中间向两侧拨开，山峦在抖动中模糊，随着手势向两侧移开，一条石板路从空中渐显、缓缓下沉。参展者持续奏响音符后，山体从对准的位置开始，由半透明流动态逐渐固化为坚实的青青山体，漂浮的碎石同步落定，形成可通行的石阶。

在这一过程中，水滴音符从单音发展为完整的旋律短句；风声变得柔和，清脆的水滴声融入环境音。旅人胸口的光核第一次稳定亮起，浮现水滴形印记。参展者需要按照指定的节奏挥动手——若节奏错误，山峦会退回透明，道路将再次消失，形成"尝试—反馈—修正"的交互循环。

完成固化后，系统提示："你获得了\textbf{水滴音符}——奏响它，可让流动、未定的世界元素固化成型，为自己锚定前行的路标。"旁白随之响起："并非世界无形，是你的呼唤还未曾抵达。"
...
\begin{itemize}
\item \textbf{分层日志系统}：系统采用 Python 标准库 \texttt{logging} 模块实现分层日志记录，按模块划分为 \texttt{Server} 与 \texttt{智能体} 两个独立的日志命名空间。日志同时输出至控制台与文件，采用时间戳 + 模块名 + 级别的格式，便于事后分析与问题追溯。日志文件按启动时间自动命名（\texttt{echoagent\_YYYYMMDD\_HHMMSS.log}），保存于开发项目的 \texttt{logs/} 目录下。
...
\item \textbf{情绪感知架构的局限性}：当前面部识别模块仅输出MediaPipe的52维形变系数（blendshapes）原始数据，情绪分类与理解完全交由EchoAgent完成。这一设计降低了感知层的复杂度，并使情绪推理能够结合上下文进行深层语义理解，但也带来了两方面代价：其一，智能体推理延迟在秒级，无法满足实时灯光/着色器等毫秒级响应场景的需求；其二，形变系数为底层几何特征，智能体对"中性/混合表情"的边界判断缺乏稳定锚点。后续改进可在感知层增加一个轻量级快通路（如基于形变系数的规则映射），为下游提供低延迟的情绪倾向信号，同时保留智能体的深度理解通路，形成快慢结合的双通道情绪感知架构。
